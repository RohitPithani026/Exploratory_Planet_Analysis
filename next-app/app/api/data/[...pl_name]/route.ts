...
// Define interface for exoplanet data
        ...
        fs.createReadStream(filePath)
            ...
            .on("end", () => {
                const exoplanet = results.find(
                    (planet) =>
                        planet.pl_name.toLowerCase() === planetName.toLowerCase() ||
                        planet.pl_name.toLowerCase().replace(/\s+/g, "-") === planetName.toLowerCase()
                );
            ...
            .on("error", (err) =>
                reject(
                    NextResponse.json(
                        { error: "Failed to process CSV file", details: err.message },
                        { status: 500 }
                    )
                )
            );
